...
def test_join_game(db_helper_with_one_user: DBHelper):
    ...
    db_helper_with_one_user.register_player_if_not_exist(
        user_id=new_user_id, username=joined_username
    )

    joined_user_name, game_users_ids = db_helper_with_one_user.add_user_to_game(
        user_id=new_user_id, game_id=game_id
    )
...
def test_join_non_existing_game(db_helper_with_one_user: DBHelper):
    ...
    assert (
        db_helper_with_one_user.add_user_to_game(
            user_id=new_user_id, game_id=game_id + 1
        )
        is None
    )
...
def test_attempt_to_kick_non_existing_username(db_helper_with_one_user: DBHelper):
    ...
    assert (
        db_helper_with_one_user.kick_user(
            user_id=host_user_id, username_to_kick="non_existing_username"
        )
        is None
    )
...
def test_kicking_existing_username(db_helper_with_one_user: DBHelper):
    ...
    kicked_user_id, remaining_users = db_helper_with_one_user.kick_user(
        user_id=host_user_id, username_to_kick=joined_username
    )
