...
class ForwardedFromBot(MessageFilter):
    def filter(self, message):
        return (
            message.forward_from.username == "neuroginarium_dev_bot"
        )  # move to config or del
...
class GameplayHandler:
    ...
    def __call__(self):
        return ConversationHandler(
            entry_points=[
                ...
                MessageHandler(
                    filters.FORWARDED & filters.TEXT & ForwardedFromBot(), self.join
                ),
            ],
            states={
                LOBBY_STATE: [
    ...
    async def join(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        new_player_username, all_users = self.db_helper.add_user_to_game(
            update.message.from_user.id, game_id
        )
        for user_id in all_users:
            await context.bot.send_message(
                user_id, f"{new_player_username} joined the game."
            )
    ...
    async def start_game(
        ...
        all_users, number_of_cards = self.db_helper.start_user_game(
    ...
    async def kick_user(
        ...
        text = update.message.text
        username = text[
            text.find("/kick_user") + len("/kick_user ") :
        ]  # shoud write this format in help or something
        replay = self.db_helper.kick_user(update.message.from_user.id, username)
        if replay is None:
            await context.bot.send_message(
                update.message.from_user.id,
                "no such username in your game (try without @)",
            )
        else:
            kicked_user_id, remaining_users = replay
            await context.bot.send_message(
                kicked_user_id, f"you was kicked from the game."
            )
            for user_id in remaining_users:
                await context.bot.send_message(
                    user_id, f"{username} was kicked from the game."
                )
        return PLAYING_STATE

    async def leave_game(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ) -> int:  # shoud write about this commant in help or something
        remaining_users = self.db_helper.leave_game(update.message.from_user.id)
        await context.bot.send_message(
            update.message.from_user.id, "you left the game."
        )
        for user_id in remaining_users:
            await context.bot.send_message(
                user_id, f"{update.message.from_user.username} left the game."
            )
